...
class Users_model extends MY_Model {
	...
	private string $_algorithm_password = PASSWORD_DEFAULT;
	private array $_options_password = [];

	/**
	 * @var array
	 */
	protected array $_cache_users = [];

	private $_unique_phone = true;
	private const W_HASH_PREFIX = '$W$';
	...
	private function _hash_password( $password ) {
		...
		return W_HASH_PREFIX . base64_encode( password_hash( $password, $this->_algorithm_password, $this->_options_password ) );
	}
	...
	private function _verify_password( $password, $user_id ) {
		...
		$check = password_verify( $password, base64_decode( substr( $user->password, strlen( W_HASH_PREFIX ) ) ) );
	 ...
	 * @throws Exception
	 */
	public function add_user( $email, $password, $phone = false, $fullname = null ) {
		$this->db->trans_start();
		...
		// is string
		//
		if ( empty( $password ) or ! $this->email_check( $email ) or ! $this->phone_check( $phone ) ) {
			return false;
		}
