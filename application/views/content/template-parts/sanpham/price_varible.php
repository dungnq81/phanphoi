	...
	if($check_thuoctinh!==false){
		...
		for ($i=0;$i<=$count_thuoctinh-1;$i++) {
			$array_tt=$this->functions->array_ten_thuoctinh($meta_value,$i);
				...
				if($name_pr_thuoctinhsp=='Màu sắc' || $name_pr_thuoctinhsp=='Dung lượng'){
					$type_select='size=2';
				...
				<tbody>
					<tr>
						...
						<td class="value woo-variation-items-wrapper">
							...
							<?php if($name_pr_thuoctinhsp=='Màu sắc' || $name_pr_thuoctinhsp=='Dung lượng'){// ul mausac ?>
								<ul class="color-select-ul" id="ul-<?php echo $this->xulychuoi->to_slug_style_1($name_pr_thuoctinhsp); ?>">
									<?php 
							...
							<select class="pa_select_multi form-control " name="attribute_pa" <?php echo $type_select?> id="pa_select_multi_<?php echo $this->xulychuoi->to_slug_style_1($name_pr_thuoctinhsp); ?>">
								<?php if($name_pr_thuoctinhsp!='Màu sắc' && $name_pr_thuoctinhsp!='Dung lượng'){ ?>
			...
			<div class="variation-price">
				<?php 
					...
					foreach ($meta_value as $key=> $value) {
						...
						if($min_price!='' && $min_price > $gia  && $gia!='') { 
							$min_price = $gia;
				            if($gia_km>0){
			...
			<tbody>
				<tr>
					<?php 
						...
						if($name_pr_thuoctinhsp_one=='Màu sắc' || $name_pr_thuoctinhsp_one=='Dung lượng'){
							$type_select='size=2';
						}else{
							$type_select='';
						}
					...
					<td class="value woo-variation-items-wrapper">
						...
						<?php if($name_pr_thuoctinhsp_one=='Màu sắc' || $name_pr_thuoctinhsp_one=='Dung lượng'){// ul mausac ?>
								<ul class="color-select-ul color-select-ul-one" id="ul-<?php echo $this->xulychuoi->to_slug_style_1($name_pr_thuoctinhsp_one); ?>">
									<?php 
										$min_price = $meta_value[0][2];
										$min_price_km = $meta_value[0][3];
										if($min_price_km!='' && $min_price_km<$min_price){
										...
										if(count($meta_value)==1){
											...
											$selected_css='';
											if($min_price > $gia && $count_i!=0  && $gia!='' ) {
												$min_price = $gia;
												$selected_css='selected';
									        }else if($count_i==0){
									        	$selected_css='selected';
						...
						<select class="pa_select form-control" name="attribute_pa" <?php echo $type_select?> >
							<?php if($name_pr_thuoctinhsp_one!='Màu sắc' && $name_pr_thuoctinhsp_one!='Dung lượng'){ ?>
									<option value="0">Chọn một tùy chọn</option>
							<?php } ?>
