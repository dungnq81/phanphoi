...
include(dirname(__FILE__) . '/Asset/Asset_JSMin.php');
include(dirname(__FILE__) . '/Asset/Asset_Csscompressor.php');
include(dirname(__FILE__) . '/Asset/Asset_Cssurirewriter.php');

class Asset
{
	/**
	 * @var array Array of paths in which the css, js, images directory structure can be found, relative to $asset_url
	 */
	protected static $asset_paths = [
		'core' => 'assets/',
	];
	 ...
	 */
	protected static $default_folders = [
		'css' => 'css/',
		'js' => 'js/',
		'img' => 'images/',
	];
	...
	protected static $default_options = [
		'enabled' => TRUE,
		'combine' => (ENVIRONMENT !== 'development'),
		'min' => (ENVIRONMENT !== 'development'), //'combine' => TRUE,
		//'min' => TRUE,
		'inline' => FALSE,
		'attr' => [],
		'deps' => [],
	...
	/**
	 * the function to call when we've decided on the name
	 ...
	 */
	protected static $rendered_groups = [
		'js' => [],
		'css' => []
	];
	 ...
	 * @throws Asset_Exception
	 */
	protected static function add_group_base($group_type, $group_name, $options = [])
	{
		// Insert defaults
	 ...
	 * @throws Asset_Exception
	 */
	public static function add_group($group_type, $group_name, $files, $options = [], $combine_dep = NULL, $min_dep = NULL)
	{
		// Bit of backwards compatibity.
		// Order used to be add_group(group_type, group_name, files, enabled, combine, min)
		if (! is_array($options))
		{
			$options = ['enabled' => $options, 'combine' => $combine_dep, 'min' => $min_dep,];
		}
	 ...
	 * @throws Asset_Exception
	 */
	public static function render($group = FALSE, $inline_dep = NULL, $attr = [])
	{
	 ...
	 */
	public static function render_js($group = FALSE, $inline_dep = NULL, $attr_dep = [])
	{
		...
		foreach ($file_groups as $group_name => $file_group)
		{
			...
			{
				$filename = self::combine('js', $file_group, self::$groups['js'][$group_name]['min']);
				...
				else
				{
					...
					$ret .= self::html_tag('script', ['src' => self::$asset_url . $filepath,] + $attr, '') . PHP_EOL;
				}
			}
			else
			{
				foreach ($file_group as $file)
				{
					...
					else
					{
						...
						$ret .= self::html_tag('script', ['src' => $base . $filepath,] + $attr, '') . PHP_EOL;
					}
				}
			}
		}
	 ...
	 */
	public static function render_css($group = FALSE, $inline_dep = NULL, $attr_dep = [])
	{
		...
		foreach ($file_groups as $group_name => $file_group)
		{
			...
			{
				$filename = self::combine('css', $file_group, self::$groups['css'][$group_name]['min']);
	...
	protected static function resolve_deps($type, $group_names, $depth = 0)
	{
		...
		{
			throw new Asset_Exception("Reached depth $depth trying to resolve dependencies. " . "You've probably got some circular ones involving " . implode(',', $group_names) . ". " . "If not, adjust the config key deps_max_depth.");
		}
	 ...
	 * @param string|bool $group groups to render. If false, takes all groups
	 *
	...
	protected static function files_to_render($type, $group)
	{
		...
		foreach ($group_names as $group_name)
		{
			...
			foreach (self::$groups[$type][$group_name]['files'] as $file_set)
			{
				...
				{
					array_push($files[$group_name], ['file' => $file, 'minified' => $minified,]);
				}
			}
		}
	 ...
	 * @throws Asset_Exception
	 */
	protected static function combine($type, $file_group, $minify)
	{
	 ...
	 * @throws Asset_Exception
	 */
	public static function img($images, $alt, $attr = [])
	{
		if (! is_array($images))
		{
			$images = [$images];
